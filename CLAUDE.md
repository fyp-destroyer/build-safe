...
- Missing safety-critical follow-up answers escalate risk (treat as worst plausible case), never assume safety. A follow-up has **four** states and they are all distinct: `true` (safe condition holds), `false` (it does not), `"unsure"` (the user does not know), and ABSENT. "Unsure" and absent both escalate to `floorWhenMissing`; "no" escalates to `floorWhenDenied`. Anything unrecognised is treated as absent — never as safe.
...
### Two files with unusual provenance

- **`convex/ai/classifier/model.json`** — GENERATED by `ml/export_model_json.py`
  from the trained scikit-learn artifact. Never hand-edit; CI checks it against
  scikit-learn's own predictions (`tools/compare_classifiers.mjs`).
- **`convex/ai/ruleEngine/catalog.ts`** — originally generated from the Python
  catalog, but that source no longer exists, so it is now **the source of truth
  and is edited by hand**. `tools/generate_catalog_ts.py` is kept only as a
  record of how the data first got here; re-running it would fail.

Changing the catalog changes what the product tells people is safe, and it does
so silently — a floor typed as 3 instead of 4 compiles and looks fine. Two things
guard it:

1. `npm run test` — property tests in `ruleEngine.test.ts`, including a
   **dead-parameter guard** asserting every follow-up's `floorWhenDenied` can
   actually change an outcome. Two shipped without that being true.
2. `npm run verify:rules` — replays 2,895 evaluations against a reviewed
   baseline. If a change is intended, re-baseline with
   `npx tsx ../../tools/compare_rule_engines.mjs --update` and **read the diff**:
   that diff is the safety review.
