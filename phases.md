...
- [x] **Catalog extended to 21 rules (2026-07-31)**, the last three under a held-out validation protocol (`ml/data/holdout_rules.json`, committed before the rules; 14 of its 24 cases are adversarial negatives). **24/24 held out.** Deployed high-risk recall **0.902**, over-escalation 0.094, **1.000 on tasks whose hazard is stated** — see `ml/analyze_recall.py` and `tools/check_holdout.mjs`.
